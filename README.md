...
基于 Spring AI 的 Advisor 链，在 LLM 调用两端各设一道**确定性**护栏：

- **输入护栏**（链最外层，order = `Integer.MIN_VALUE`）—— 拦截 prompt 注入、角色劫持、
  系统提示词刺探、评分操纵。命中即短路，**LLM 根本不被调用**（省 token），
  且被拦消息不写入对话历史（避免注入内容成为后续越狱的上下文）。
- **输出护栏**（链最内层，order = `Integer.MAX_VALUE`）—— 检测系统提示词泄漏与
  PII（手机号 / 身份证 / API Key）。流式场景用**滑动窗口**检测，可捕获跨 chunk 边界的匹配。

规则为确定性正则，无额外 LLM 往返，因此**不增加调用延迟**。

**已知局限（不粉饰）**：
- 确定性规则可被语义变体绕过。防护目标是抬高攻击成本、拦住明显越狱，不是完备防护
- 流式输出下，已推送给前端的 chunk 无法收回，只能截断剩余流 —— 这是为保留流式体验
  而做的刻意取舍
- 系统提示词泄漏检测依赖片段字面量匹配，**改动 system prompt 时必须同步更新
  `OUTPUT_PROMPT_FRAGMENTS`**，否则会静默失效（片段对不上不会报错，只是永远匹配不到）
