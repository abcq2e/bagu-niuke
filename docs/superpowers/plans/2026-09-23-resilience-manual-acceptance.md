...
| 2 | 日志出现 `✅ 已降级至备模型并成功返回` | ✅ **出现 3 次**（第一轮因 DashScope 欠费未通过，充值并关闭「免费额度用完即停」后通过） |
| 3 | 主备都坏时用户收到 `[ERROR] AI 服务暂时不可用`，日志有 ERROR 级记录 | ✅ `❌ 备模型也失败，返回兜底话术` 出现（第一轮的 3 次） |
| 4 | 重启服务后面试官仍记得项目描述 | ⬜ 未验收（需要前端交互） |
| 5 | 断线时日志出现 `🔌 客户端断线` | ⬜ 未验收（需要真实浏览器断线） |
| 6 | 未改动任何 `🎯 Task N` 骨架文件 | ✅ 全程 `grep "🎯 Task"` 零命中 |

**两条路径都跑到了**：第一轮（备模型不可用）验证了「主备都挂 → 兜底话术」；
第二轮（备模型可用）验证了「主模型挂 → 备模型接管并成功返回」。合起来覆盖了计划要求的全部三种情况。
...
**第一轮现象**：降级被正确触发了，但备模型自己也是坏的，报
`HTTP 400 - {"code":"InvalidParameter","message":"Model not exist."}`。

**排查过程**（值得记，因为错误码指向的方向是错的）：

| 直接探测 DashScope 接口时的返回 | 含义 |
|---|---|
| `{"code":"Arrearage"}` | 账户欠费 |
| `AllocationQuota.FreeTierOnly` | 免费额度耗尽 **且** 控制台开着「免费额度用完即停」开关 |
| （充值后）正常返回 | ✅ |

**关键教训**：应用日志报的 `Model not exist` **具有误导性** ——
模型名 `qwen-plus` 一直是正确的，真正的问题是账号侧配额。
排查这类问题，**直接拿 key 打一遍厂商接口**比看应用日志快得多。

**解决动作**：
1. 给阿里云账户充值
2. 在百炼控制台的**「免费额度」**页面**关闭对应模型的「免费额度用完即停」开关**
   （未绑定支付方式的账号该开关默认为开且不可关，需先完成支付方式验证）
   —— **只充值不关这个开关是无效的**

**第二轮结果**：`✅ 已降级至备模型并成功返回` 出现 3 次，备模型真正接管了回答。

**残留观察**：第二轮里仍有零星 `Model not exist`，来自 DashScope。
模型名与 key 都验证过是对的，判断是**账号配额在该次运行中再次触顶**。
若将来频繁出现，先查百炼控制台的用量，不要先怀疑代码。
